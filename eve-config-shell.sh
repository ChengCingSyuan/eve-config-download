#!/usr/bin/env bash
#
# EVE-Config-Shell - EVE-NG 配置脚本
# 版本：v0.11
# 本项目及其源代码遵循 GNU General Public License v3.0(GPLv3) 协议发布
...
MS_INTEL_URL="https://jen5-my.sharepoint.com/personal/cingsyuan_jen5_onmicrosoft_com/_layouts/52/download.aspx?share=IQCs28y_XkRgQLcXwJAFE7XaAWXQ5poXkSTFm04W1VoTdpg"
...
# hi168 源（H3C）
HI168_H3C_FW_URL="https://s3.hi168.com/hi168-18902-7015cman/H3C/h3cvfw1k-7.1.064-E1260P45.tgz"
HI168_H3C_ROUTE_URL="https://s3.hi168.com/hi168-18902-7015cman/H3C/h3cvsr1k-7.1.064-R1340P22.tgz"
HI168_H3C_SWITCH_URL="https://s3.hi168.com/hi168-18902-7015cman/H3C/h3cvswitch_S9850-7.1.070-R7643P02.tgz"

# 微软源（H3C）
MS_H3C_FW_URL="https://jen5-my.sharepoint.com/personal/cingsyuan_jen5_onmicrosoft_com/_layouts/52/download.aspx?share=IQACT8654L6HRpq5BeTybFqeAbiX7qkbsjSq526urlhyZds"
MS_H3C_ROUTE_URL="https://jen5-my.sharepoint.com/personal/cingsyuan_jen5_onmicrosoft_com/_layouts/52/download.aspx?share=IQAjdDRQzyvBTJDaY6QAWk50AROkd2jXv-iUU1_AqDNkfXc"
MS_H3C_SWITCH_URL="https://jen5-my.sharepoint.com/personal/cingsyuan_jen5_onmicrosoft_com/_layouts/52/download.aspx?share=IQAAyvhwr51ZSp0C8PWiwIZEAaL1C6SLTUumjP-9krQPE2M"
...
# H3C 当前源 URL（根据源切换）
H3C_FW_URL="$HI168_H3C_FW_URL"
H3C_ROUTE_URL="$HI168_H3C_ROUTE_URL"
H3C_SWITCH_URL="$HI168_H3C_SWITCH_URL"
...
update_system_info() {
    ...
    if [[ -f /etc/os-release ]]; then
        # 这些变量在函数内是 local，不会覆盖脚本最上面的 VERSION
        local NAME PRETTY_NAME VERSION VERSION_ID VERSION_CODENAME
        # shellcheck disable=SC1091
        . /etc/os-release

        if [[ "$NAME" == "Ubuntu" && -n "$VERSION" ]]; then
            # 例如：Ubuntu 22.04.5 LTS (Jammy Jellyfish)
            SYSTEM_OS="${NAME} ${VERSION}"
...
check_is_ubuntu_jammy_or_confirm() {
    if [[ -f /etc/os-release ]]; then
        # 在函数内 local 掉这些变量，避免覆盖脚本顶部的 VERSION
        local NAME VERSION VERSION_ID VERSION_CODENAME
        # shellcheck disable=SC1091
        . /etc/os-release
...
set_source_ms() {
    ...
    # H3C 镜像
    H3C_FW_URL="$MS_H3C_FW_URL"
    H3C_ROUTE_URL="$MS_H3C_ROUTE_URL"
    H3C_SWITCH_URL="$MS_H3C_SWITCH_URL"
...
# 通用：下载 + 解压 + 复制到 QEMU_DIR/指定目录（H3C 用）
download_and_install_image_qemu_to_dir() {
    local label="$1"     # 说明文案
    local url="$2"
    local outfile="$3"   # 下载后文件名，如 xxx.tgz
    local dst_dir="$4"   # 目标目录名，例如 h3cvac1k-7.1.064-R5482
    ...
    local target="${QEMU_DIR}/${dst_dir}"
    mkdir -p "$target"
    echo "[*] 正在复制内容到：${target}"
    cp -a "${unpack_dir}/." "${target}/"
...
install_h3c_images() {
    ...
    echo "[*] 将安装以下 H3C 镜像："
    echo "    1) H3C 防火墙   -> 目录名：h3cvfw1k-7.1.064-E1260P45"
    echo "    2) H3C 路由器   -> 目录名：h3cvsr1k-7.1.064-R1340P22"
    echo "    3) H3C 交换机   -> 目录名：h3cvswitch_S9850-7.1.070-R7643P02"
    echo
    read -rp "[?] 是否全部安装以上三个 H3C 镜像？[Y/n]: " ans
    ans=${ans:-Y}
    if [[ ! "$ans" =~ ^[Yy]$ ]]; then
        echo "[*] 已取消 H3C 镜像安装。"
        return 0
    ...
    # 防火墙：目标目录 h3cvfw1k-7.1.064-E1260P45
    download_and_install_image_qemu_to_dir "H3C 防火墙 (h3cvfw1k-7.1.064-E1260P45)" \
        "$H3C_FW_URL" "h3cvfw1k-7.1.064-E1260P45.tgz" "h3cvfw1k-7.1.064-E1260P45" \
        || { echo "[-] H3C 防火墙镜像安装失败。"; rm -rf "$TMP_DIR"; return 1; }

    # 路由器：目标目录 h3cvsr1k-7.1.064-R1340P22
    download_and_install_image_qemu_to_dir "H3C 路由器 (h3cvsr1k-7.1.064-R1340P22)" \
        "$H3C_ROUTE_URL" "h3cvsr1k-7.1.064-R1340P22.tgz" "h3cvsr1k-7.1.064-R1340P22" \
        || { echo "[-] H3C 路由器镜像安装失败。"; rm -rf "$TMP_DIR"; return 1; }

    # 交换机：目标目录 h3cvswitch_S9850-7.1.070-R7643P02
    download_and_install_image_qemu_to_dir "H3C 交换机 (h3cvswitch_S9850-7.1.070-R7643P02)" \
        "$H3C_SWITCH_URL" "h3cvswitch_S9850-7.1.070-R7643P02.tgz" "h3cvswitch_S9850-7.1.070-R7643P02" \
        || { echo "[-] H3C 交换机镜像安装失败。"; rm -rf "$TMP_DIR"; return 1; }
