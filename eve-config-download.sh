...
EVE-NG 国代官方提供相关技术支持。
项目地址 https://github.com/ChengCingSyuan/eve-config-download
============================================================
...
check_requirements() {
    command -v wget >/dev/null 2>&1 || {
        echo "[-] 未找到 wget，请先安装：apt update && apt install -y wget"
        exit 1
    }
    command -v tar >/dev/null 2>&1 || {
        echo "[-] 未找到 tar，请先安装：apt update && apt install -y tar"
        exit 1
    }
}
...
check_requirements_ruijie() {
    command -v aria2c >/dev/null 2>&1 || {
        echo "[-] 未找到 aria2c（多线程下载工具）。"
        echo "    请先安装：apt update && apt install -y aria2"
        exit 1
    }
    command -v tar >/dev/null 2>&1 || {
        echo "[-] 未找到 tar，请先安装：apt update && apt install -y tar"
...
#========================#
# APT 源切换与工具安装模块（菜单 100）
#========================#

maybe_update_apt_and_install_tools() {
    echo
    echo "============================================================"
    echo " 100 - 安装必要组件（APT 源切换 + 常用工具）"
    echo "============================================================"
    read -rp "[?] 是否将 Ubuntu 22.04 APT 源切换为阿里云并安装常用工具 (vim sudo curl wget aria2 tar)？[Y/n]: " ans
    ans=${ans:-Y}
    if [[ ! "$ans" =~ ^[Yy]$ ]]; then
        echo "[*] 已选择不修改 APT 源，跳过该步骤。"
        return 0
    fi

    # 简单检测当前是否为 Ubuntu 22.04 / jammy
    if [[ -f /etc/os-release ]]; then
        # shellcheck disable=SC1091
        . /etc/os-release
        if [[ "${VERSION_ID}" != "22.04" && "${VERSION_CODENAME}" != "jammy" ]]; then
            echo "[!] 检测到当前系统并非 Ubuntu 22.04 (jammy)，"
            echo "    使用阿里云 jammy 源可能导致问题，请自行确认。"
    ...
    if [[ ! -f /etc/apt/sources.list.bak_eve_config ]]; then
        ...
        echo "[*] 已存在备份 /etc/apt/sources.list.bak_eve_config，跳过再次备份。"
    fi
...
EOF
    ...
    echo "[*] 已写入阿里云 Ubuntu 22.04 (jammy) APT 源。"
    echo "[*] 正在执行 apt update ..."
    apt update

    echo "[*] 正在安装常用工具：vim sudo curl wget aria2 ..."
    apt install -y vim sudo curl wget aria2 tar
